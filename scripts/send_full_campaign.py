...
# 3. Collect and Filter High-Deliverability Verified Prospects
all_files = [
...
for l in raw_leads:
    em = l['email'].lower()
    if em not in seen_emails and em not in opt_outs:
        seen_emails.add(em)
        unique_leads.append(l)

# Prioritize direct inboxes (gmail, yahoo, outlook, hotmail, official verified clinic inboxes)
# Discard high-risk generic corporate role emails (e.g. info@apollohospitals.com)
high_deliverability = []
for l in unique_leads:
    em = l['email']
    domain = em.split('@')[1]
    # Filter out known bounce-heavy corporate enterprise domains
    if domain in ['apollohospitals.com', 'fortishealthcare.com', 'maxhealthcare.com', 'manipalhospitals.com', 'medanta.org']:
        continue
    high_deliverability.append(l)

target_batch = high_deliverability[:110]
print("======================================================")
print(f"🏥 SWASTHAI LIVE OUTREACH BATCH DISPATCH (BREVO)")
print("======================================================")
print(f"• Total Filtered Verified Leads: {len(high_deliverability)}")
print(f"• Target Dispatch Count:         {len(target_batch)} emails")
print(f"• Sender Address:                Sankalp Mishra <{brevo_sender}>")
print(f"• Campaign:                      scan_register_25cr_milestone")
print("======================================================\n")

# 4. Load Existing Send Log
log_entries = []
...
for idx, lead in enumerate(target_batch, 1):
    ...
    website_url = "https://swasthai-three.vercel.app/?utm_source=email&utm_medium=cold_outreach&utm_campaign=scan_register_25cr_milestone"
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; font-size: 15px; line-height: 1.6; color: #222222; background-color: #ffffff; margin: 0; padding: 20px 0;">
  <div style="max-width: 600px; margin: 0 auto; padding: 0 20px;">
    ...
    payload = {
        ...
        "tags": ["scan_register_25cr_milestone", f"rank_{idx}"]
    }
...
print("\n======================================================")
print("🎯 SWASTHAI OUTBOUND BATCH COMPLETE")
print("======================================================")
print(f"• Total Successfully Delivered: {sent_count}")
